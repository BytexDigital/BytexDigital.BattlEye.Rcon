    ...
    public class NetworkMessageHandler
    {
        ...
        public void Cleanup()
        {
            lock (_networkRequests)
            {
                ...
                foreach (var toRemove in toRemoves) RemoveRequest(toRemove);
            }
        }
        ...
        private void HandleMessagePacket(IEnumerable<byte> data)
        {
            ...
            const string playerRemovedPattern =
                @"Player #(\d+) (.+) \((\S{32})\) has been kicked by BattlEye: Admin (Kick|Ban)(?: \((.+)\))?";
            ...
            if (Regex.IsMatch(content, playerConnectedPattern))
            {
                foreach (Match match in Regex.Matches(content, playerConnectedPattern))
                {
                    ...
                    try
                    {
                        _networkConnection.FireProtocolEvent(
                            new GenericParsedEventArgs(new PlayerConnectedArgs(id, guid, name)));
                    }
                    catch
                    {
                    }
            ...
            if (Regex.IsMatch(content, playerDisconnectedPattern))
            {
                foreach (Match match in Regex.Matches(content, playerDisconnectedPattern))
                {
                    ...
                    {
                        _networkConnection.FireProtocolEvent(
                            new GenericParsedEventArgs(new PlayerDisconnectedArgs(id, name)));
                    }
                    catch
                    {
                    }
            ...
            if (Regex.IsMatch(content, playerRemovedPattern))
            {
                foreach (Match match in Regex.Matches(content, playerRemovedPattern))
                {
                    ...
                    var isBan = match.Groups[4].Value == "Ban";
                    var reason = match.Groups.Count > 5 ? match.Groups[5].Value : null;

                    try
                    {
                        _networkConnection.FireProtocolEvent(
                            new GenericParsedEventArgs(new PlayerRemovedArgs(id, guid, name, isBan, reason)));
                    }
                    catch
                    {
                    }
        ...
        private void HandleCommandResponse(IEnumerable<byte> data)
        {
            ...
            if (payload.Count() > 0)
            {
                ...
                {
                    // Multi-part message
                    if (request.Response == null) request.SetResponse(new CommandNetworkResponse(""));

                    var expectedAmount = data.Skip(2).First();
                    var currentIndex = data.Skip(3).First();
                    var partData = data.Skip(4);

                    var isLastPartOfMessage = expectedAmount == currentIndex + 1;
